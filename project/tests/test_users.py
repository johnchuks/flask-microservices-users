...
class TestUserService(BaseTestCase):
    ...
    def test_get_single_user(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('itunu', data['data']['username'])
            self.assertIn('itunu@gmail.com', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_add_users(self):
        ...
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username='johnB', email='johnb@realpython.com'),
                follow_redirects=True
            )
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'<h1>All Users</h1>', response.data)
            self.assertNotIn(b'<p>No users</p>', response.data)
            self.assertIn(b'<strong>johnB</strong>', response.data)
